    ...
    for between_user_technique in between_user_data_reduction_techniques:
        ...
        for ind, retained_parcent in enumerate(retained_user_parcentages):
            ...
            for fold in range(spliter.k_fold):
                ...
                saved_dir = nnRecModel.saved_dir + 'epistemic_uncertainty/' + between_user_technique + '/%.2f/best_models/'%(retained_parcent) + 'fold_%i_best_model'%(fold)
                # Load weights to model
                print(saved_dir)
                model.load_weights(saved_dir)

                prediction_probabilities, prediction_variances = Models.montecarlo_prediction(model, test, T=10)

                user_epistemic_uncertainty[test.users.squeeze()] = prediction_variances
                hr = evaluate_model(model, test, T=10, requireAleatoricUncertainty=False)

                hr_metrics[ind, fold, :] = np.array([hr, np.average(prediction_variances)])

            location = nnRecModel.saved_dir + 'epistemic_uncertainty/' + between_user_technique + '_user_ep_un_%.2f' % (retained_parcent)
            with open(location, 'wb') as f:
                pickle.dump(user_epistemic_uncertainty, f)

        with open(nnRecModel.saved_dir + 'epistemic_uncertainty/' + between_user_technique + '_metrics', 'wb') as f:
