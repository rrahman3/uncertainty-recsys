...
def montecarlo_prediction(model, test, T=100):
    ...
    softmax_y_pred = tf.nn.softmax(y_pred) #(batch_size, num_items)
...
class NextItNet_DistributedWeights(nnRecSys):
    def __init__(self, args, data=None, sequence_length=10, T=10):
        super(NextItNet_DistributedWeights, self).__init__(data, sequence_length, T)
        self.saved_dir = 'results/NextItNet_DistributedWeights/' + args.dataset + '/'
...
class NextItNet2D_DistributedWeights(nnRecSys):
    def __init__(self, args, data=None, sequence_length=10, T=10):
        super(NextItNet2D_DistributedWeights, self).__init__(data, sequence_length, T)
        self.saved_dir = 'results/NextItNet2D_DistributedWeights/' + args.dataset + '/'
        self.embed_dim = args.embed_dims                    # 256
        self.residual_channels = args.residual_channels     # 256
        self.dilations = args.dilations                     # [1, 4, 1, 4, 1, 4, 1, 4, 1, 4, 1, 4, 1, 4]
        self.kernel_size = args.kernel_size            # 3
    ...
    def get_model(self, drop_prob=0.25, isDropoutTraining=True):
        ...
        # flatten_layer = tf.reshape(conv_layer[:, -1, ], (-1, self.residual_channels))  # (batch_size, embed_dim) = (None, 32)
        flatten_layer = conv_layer[:, -1, ] # (batch_size, embed_dim) = (None, 32)
        flatten_layer = tf.keras.layers.Flatten()(flatten_layer)
...
class NextItNet_Dropout(nnRecSys):
    def __init__(self, data=None, sequence_length=10, T=10):
        super(NextItNet_Dropout, self).__init__(data, sequence_length, T)
        self.saved_dir = 'results/NextItNet_Dropout/'
    ...
    def get_model(self, drop_prob=0.25, isDropoutTraining=True):
        ...
        return self.compile_model(inputs=seq_input, outputs=output_layer)


class GRU_Dropout(nnRecSys):
    def __init__(self, args, data=None, sequence_length=10, T=10):
        super(GRU_Dropout, self).__init__(data, sequence_length, T)
        self.saved_dir = 'results/GRU_Dropout/' + args.dataset + '/'
        self.embed_dim = args.embed_dims                    # 256
        self.residual_channels = args.residual_channels     # 256
        self.dilations = args.dilations                     # [1, 4, 1, 4, 1, 4, 1, 4, 1, 4, 1, 4, 1, 4]
        self.kernel_size = args.kernel_size            # 3
    ...
    def get_model(self, drop_prob=0.25, isDropoutTraining=True):
        ...
        item_embedding_dropout = tf.keras.layers.Dropout(drop_prob)(item_embedding, training=isDropoutTraining)
        gru_layer = tf.keras.layers.GRU(self.embed_dim)(item_embedding_dropout)
        print('gru_layer.shape:', gru_layer.shape)

        gru_dropout = tf.keras.layers.Dropout(drop_prob)(gru_layer, training=isDropoutTraining)
        print('dropout_layer.shape:', gru_dropout.shape)

        fully_connected_layer = tf.keras.layers.Dense(self.embed_dim, activation=tf.nn.relu)(gru_dropout)
        print('fully_connected_layer.shape:', fully_connected_layer.shape)

        fully_connected_dropout = tf.keras.layers.Dropout(drop_prob)(fully_connected_layer, training=isDropoutTraining)
